...
def fetch_cached_history(symbol, period="2y", interval="1d"):
    ...
    if os.path.exists(file_path):
        ...
        if age_days > config.MAX_CACHE_AGE_DAYS:
            ...
            try: df = pd.read_csv(file_path, index_col=0, parse_dates=True)
    ...
    except Exception as e: logger.warning("Cache save failed for %s: %s", symbol, e)
    return df
...
def send_email(subject, body):
    ...
    except Exception as e: logger.error("Email failed: %s", e)
...
def job():
    ...
    for symbol in tickers:
        ...
        try: rt_price = finnhub_client.quote(symbol).get("c", price)
        except Exception: rt_price = price
    ...
    logger.info("SUMMARY: Processed=%d, Skipped=%d, Alerts=%d", total, skipped, len(rsi_alerts))
    print(email_body)
