...
if datetime.datetime.today().weekday() == 6:  # Sunday = 6
    logger.info("Refreshing tickers (Sunday update)")
    try:
        ...
        logger.info("Ticker refresh done. Count=%d", len(all_tickers))
...
def fetch_cached_history(symbol, period="2y", interval="1d"):
    ...
    if os.path.exists(file_path):
        ...
        if age_days > config.MAX_CACHE_AGE_DAYS:
            logger.info("%s cache expired (%d days old). Full refresh.", symbol, age_days)
            force_full = True
        else:
            try:
    ...
    try:
        ...
        logger.warning("Save cache failed for %s: %s", symbol, e)
...
def generate_rsi_signal(df):
    ...
    if pd.notna(rsi):
        if rsi < config.RSI_OVERSOLD:
            sig, reason = "BUY", f"RSI={rsi:.1f} < {config.RSI_OVERSOLD}"
        elif rsi > config.RSI_OVERBOUGHT:
            sig, reason = "SELL", f"RSI={rsi:.1f} > {config.RSI_OVERBOUGHT}"
    return sig, reason, rsi, price
...
def job():
    ...
    for symbol in tickers:
        ...
        if sig:
            line = (f"{symbol}: {sig} at ${rt_price:.2f}, {reason}, "
                    f"PE={pe or 'N/A'}, MarketCap={mcap or 'N/A'}")
            if iv_rank: line += f", IV Rank={iv_rank}, IV Percentile={iv_pct}"
            ...
            log_alert({
                ...
                "rsi": round(rsi,2) if rsi else None, "pe_ratio": pe,
                "market_cap": mcap, "iv_rank": iv_rank, "iv_percentile": iv_pct
            })
