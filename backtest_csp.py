...
def backtest_csp(
    ...
    today = dt.date.today()
    start_bt = today - dt.timedelta(days=365 * 2)
    hist_start = start_bt - dt.timedelta(days=370)
    end_bt = today
    ...
    cash = start_cash
    reserved = 0.0
    open_csp = {t: None for t in tickers}
    ledger = []
    trades = []
        ...
        for t in tickers:
            ...
            if current_date.date() >= pos["expiry"].date():
                ...
                if current_date not in df.index:
                    ...
                    S_T = pos["S_at_open"] if len(prev) == 0 else float(df.loc[prev[-1], "Close"])
                else:
                    S_T = float(df.loc[current_date, "Close"])

                intrinsic = max(pos["K"] - S_T, 0.0)
    ...
    for entry in ledger:
        assert 'date' in entry, f"Missing 'date' in ledger entry: {entry}"
